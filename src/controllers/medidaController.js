...
// Função para buscar as porcentagens dos acertos 12/07
function buscarPorcentagem(req, res) {
    console.log('Recuperando as porcentagens de acertos');

    medidaModel.buscarPorcentagem().then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!");
